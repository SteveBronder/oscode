...
template <typename Sol> PyObject *to_result_dict(Sol &&solution) {
  // Build output values
  auto pyx_eval = to_pylist(std::forward<Sol>(solution).dosol);
  auto pydx_eval = to_pylist(std::forward<Sol>(solution).dodsol);
  auto pysol = to_pylist(std::forward<Sol>(solution).sol);
  auto pydsol = to_pylist(std::forward<Sol>(solution).dsol);
  auto pytimes = to_pylist(std::forward<Sol>(solution).times);
  auto pywkbs = to_pylist(std::forward<Sol>(solution).wkbs);
  auto pycts_rep = to_pylist(std::forward<Sol>(solution).sol_vdm);
...
static PyObject *_pyoscode_solve(PyObject *self, PyObject *args,
  ...
  using loglinear_interp_t =
      ExpInterpolator<double *, std::complex<double> *, double *>;
  if (t_evalobj != NULL) {
    if (islogw && islogg) {
      ...
      de_sys_t sys(ts, ws, gs, ts, tssize, std::move(winterp), std::move(ginterp), even, check_grid);
      check_sys(sys);
      Solution<de_sys_t> solution(sys, x0, dx0, ti, tf, t_evallist, order, rtol, atol, h0,
                                  full_output);
      solution.solve();
      return to_result_dict(solution);
    } else if (islogw) {
    ...
    if (islogw && islogg) {
      ...
      de_sys_t sys(ts, ws, gs, ts, tssize, std::move(winterp), std::move(ginterp), even, check_grid);
      check_sys(sys);
      Solution<de_sys_t> solution(sys, x0, dx0, ti, tf, order, rtol, atol, h0, full_output);
      solution.solve();
      return to_result_dict(solution);
    } else if (islogw) {
      loglinear_interp_t winterp(ts, ws, even);
