 ...
 */
template <typename Sys> class RKSolver {
public:
  /** Defines the ODE */
  Sys *de_sys_;
  // grid of ws, gs_
  ...
  Eigen::Matrix<std::complex<double>, 6, 2> ws_gs_;
...
public:
   ...
   */
  RKSolver(Sys &de_sys) : de_sys_(&de_sys) {}
  ...
  step(std::complex<double> x0, std::complex<double> dx0, double t0, double h) {
    ...
    ws_gs_(0, 0) = de_sys_->w_func_(t0);
    ws_gs_(0, 1) = de_sys_->g_func_(t0);
    k5_.col(0) = h * f(ws_gs_(0, 0), ws_gs_(0, 1), y0);
    ...
    for (int s = 1; s < 6; s++) {
      ...
      const auto next_step = t0 + butcher_c5(s) * h;
      ws_gs_(s, 0) = de_sys_->w_func_(next_step);
      ws_gs_(s, 1) = de_sys_->g_func_(next_step);
      k5_.col(s) = h * f(ws_gs_(s, 0), ws_gs_(s, 1), y);
    }
    k4.col(0) = k5_.col(0);
    ws5_(0) = ws_gs_(0, 0);
    gs5_(0) = ws_gs_(0, 1);
    for (int s = 1; s < 4; s++) {
      ...
      const auto next_step = t0 + butcher_c4(s) * h;
      ws5_(s) = de_sys_->w_func_(next_step);
      gs5_(s) = de_sys_->g_func_(next_step);
      k4.col(s) = h * f(ws5_(s), gs5_(s), y);
    }
    ...
    ws5_(2) = de_sys_->w_func_(next_step);
    gs5_(2) = de_sys_->g_func_(next_step);

    // Fill up k_dense matrix for dense output
    k_dense_.block<2, 6>(0, 0) = k5_.block<2, 6>(0, 0);
    const auto final_step = t0 + h;
    auto wi = de_sys_->w_func_(final_step);
    auto gi = de_sys_->g_func_(final_step);
    k_dense_.col(6) = h * f(wi, gi, result.col(0));
  ...
  Eigen::Matrix<std::complex<double>, 1, 2> f(std::complex<double> wi, std::complex<double> gi, const Mat &y) {
    return Eigen::Matrix<std::complex<double>, 1, 2>{
        {y[1], -wi * wi * y[0] - 2.0 * gi * y[1]}};
  }
  /** Gives dense output at a single point during the step "for free", i.e. at
   * no extra evaluations of \f$ \omega(t), \gamma(t) \f$. This solution is
   * roughly mid-way through the step at $\sigma \sim 0.59 \f$, where \f$ \sigma
   * = 0 \f$ corresponds to the start, \f$ \sigma = 1 \f$ to the end of the
   * step.
   */
  ...
  void dense_step(double t0, double h0, std::complex<double> y0,
    ...
    R_dense.col(0).array() = (Eigen::Map<const Eigen::Array<double, -1, 1>>(
                                  dots.data(), dots.size()) -
                              t0) /
                             h0;
    ...
    Eigen::Matrix<std::complex<double>, -1, 2> Y_dense =
        R_dense * (k_dense_ * P_dense_).transpose();
    Eigen::Map<Eigen::Array<std::complex<double>, -1, 1>> doxs_vec(doxs.data(),
                                                                   doxs.size());
    Eigen::Map<Eigen::Array<std::complex<double>, -1, 1>> dodxs_vec(
        dodxs.data(), dodxs.size());
