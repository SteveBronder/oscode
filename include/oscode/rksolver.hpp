...
public:
  ...
  RKSolver(Sys &de_sys) {
    ...
    butcher_a5 << 0.1174723380352676535740, 0, 0, 0, 0,
    ...
    butcher_a4 << 0.172673164646011428100, 0, 0, -1.568317088384971429762,
        2.395643923738960001662, 0, -8.769507466172720011410,
        10.97821961869480000808, -1.208712152522079996671;
    butcher_b5 << 0.1127557227351729739820, 0, 0.5065579732655351768382,
    ...
    butcher_b4 << -0.08333333333333333333558, 0.5833333333333333333357,
        0.5833333333333333333356, -0.08333333333333333333558;
    butcher_c4 << 0, 0.172673164646011428100, 0.827326835353988571900, 1;
    P_dense << 1., -2.48711376, 2.42525041, -0.82538093, 0., 0., 0., 0., 0.,
        3.78546138, -5.54469086, 2.26578746, 0., -0.27734213, 0.74788587,
  ...
  step(std::complex<double> x0, std::complex<double> dx0, double t0, double h) {
    ...
    gs5(4) = gs5(3);
    gs5(3) = gs5(2);
    ws5(2) = de_sys_->w_func_(t0 + h / 2);
    gs5(2) = de_sys_->g_func_(t0 + h / 2);
  ...
  f(double t, const Eigen::Matrix<std::complex<double>, 1, 2> &y) {
    ...
    //    std::endl;
    wi = de_sys_->w_func_(t);
    gi = de_sys_->g_func_(t);
